...
def main():
    ...
    gamma = 0.995
    eps_clip = 0.2
    action_std_init = 0.8  # Continue with a smaller exploration for fine-tuning
    action_std_decay_rate = 0.015  
    min_action_std = 0.1
    
    # Model names
    phase2_model_name = "otter_MLPPPO_imazu_01_phase2_BEST"
    phase3_model_name = "otter_MLPPPO_imazu_02_phase3"
    ...
    if load_model:
        ...
        try:
            ...
            print(f"   ❌ ERROR: Model file not found at 'robot_nav/models/PPO/best_checkpoint/{phase2_model_name}_policy.pth'.")
    ...
    while epoch < max_epochs:
        ...
        if terminal or steps >= max_steps:
            ...
            if episode_count % episodes_per_epoch == 0:
                epoch += 1
                avg_reward, avg_goal, avg_col = evaluate(model, epoch, phase3_worlds, nr_eval_episodes, max_steps)
            ...
            sim.world_file = random.choice(phase3_worlds)
            distance, y_e, collision, goal, a, reward, robot_state, CR_max = sim.reset()
            steps = 0
