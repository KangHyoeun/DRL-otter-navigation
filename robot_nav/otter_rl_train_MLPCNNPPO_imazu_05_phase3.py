...
def main():
    ...
    # Model names
    phase2_model_name = "otter_MLPCNNPPO_imazu_01_phase2_BEST"
    phase3_model_name = "otter_MLPCNNPPO_imazu_05_phase3"
    
    print("\n" + "=" * 60)
    print("🎯 CURRICULUM PHASE 3: COLLISION AVOIDANCE (2 Target Ships)")
    print("=" * 60)
    ...
    if load_model:
        print(f"\n🔄 Loading pre-trained model from Phase 2: {phase2_model_name}")
        try:
    ...
    sim = OtterSIM(
        world_file=random.choice(phase3_worlds),
    ...
    # =================================================================================
    if not load_model:
        print("\n🔥 Starting RMS Warmup (7000 steps)...")
        ...
        for _ in tqdm(range(warmup_steps), desc="Warmup"):
            ...
            w_steps += 1
            if w_col or w_goal:
                w_dist, w_ye, w_psi, w_chi, w_phi, w_col, w_goal, w_a, w_rew, w_state, w_cr, w_grid = w_sim.reset(world_file=random.choice(phase3_worlds))
    ...
    while epoch < max_epochs:
        ...
        if episode_count % episodes_per_epoch == 0 and episode_count > 0:
            epoch += 1
            avg_reward, avg_goal, avg_col = evaluate(model, epoch, phase3_worlds, nr_eval_episodes, max_steps, chi_inf=chi_inf, k=k)
            ...
            if avg_reward > best_reward:
                ...
                patience_counter = 0
                model.save(filename=f"{phase3_model_name}_BEST", directory=Path("robot_nav/models/PPO/best_checkpoint"))
                ...
                metrics = {
                    ...
                    "training_mode": "random_phase3",
                    "imazu_scenario": "05-11"
                }
                with open("robot_nav/models/PPO/best_checkpoint/best_metrics_imazu_05_MLPCNNPPO_phase3.json", "w") as f:
