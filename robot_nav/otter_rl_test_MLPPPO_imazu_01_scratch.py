...
def main(args=None):
    ...
    model = MLPPPO(
        ...
        device=device,
        load_model=True,
        model_name="otter_MLPPPO_imazu_01_scratch_BEST",
        load_directory=Path("robot_nav/models/PPO/best_checkpoint"),
    )

    sim = OtterSIM(
        world_file="robot_nav/worlds/imazu_scenario/imazu_case_01.yaml", disable_plotting=True, enable_phase1=True, max_steps=max_steps
    )
