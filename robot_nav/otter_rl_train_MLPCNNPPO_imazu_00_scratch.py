...
from colregs_core.utils import WrapTo180
...
def main():
    ...
    # Model names
    # This script will load the hardcoded model but save its own best model as "otter_MLPCNNPPO_imazu_00_scratch_BEST"
    model_to_load_for_transfer = "otter_MLPCNNPPO_imazu_00_hardcoded"
    model_name_for_save = "otter_MLPCNNPPO_imazu_00_scratch"
    ...
    while epoch < max_epochs:
        ...
        if episode_count % episodes_per_epoch == 0 and episode_count > 0:
            epoch += 1
            avg_reward, avg_goal, avg_col = evaluate(model, epoch, sim, nr_eval_episodes, max_steps)
            ...
            if avg_reward > best_reward:
                ...
                with open(best_dir / "best_metrics_imazu_00_MLPCNNPPO_scratch.json", 'w') as f:
    ...
    print(f"   Model name:            otter_MLPCNNPPO_imazu_00_scratch_BEST")
    print("=" * 60)


def evaluate(model, epoch, sim, eval_episodes, max_steps):
